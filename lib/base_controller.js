...
exports.BaseController = function(app) {
  var self = this;
  self.app = app;
  ...
  self.handle = function(controller_name, action, res, params, fmt) {
    ...
    var layout = this.choose(vars, "layout", "app");
    var fmts = this.choose(vars, "formats", ['html']);
    // TODO -- clean up params #
    this.render(controller_name, action, res, fmt, vars, layout, fmts);
  };

  // TODO -- move this (partially or fully) to views.js
  self.render = function(controller_name, action, res, fmt, vars, layout, fmts) {
    var vars = vars || {};
      ...
      switch (this.app.options["templating"]) {
        case "jade":
          ...
          try {
            ...
            var view_html = jade.render(this.app.views[controller_name][action], {locals: vars});
            var locals = "";
            // if (view_err) {
            ...
            try {
              ...
              app.crash("Jade rendering layout failed -- locals set:" +
                  locals + ".", res, err);
            }
          } catch (err) {
  ...
  self.redirect = function(res, controller_name, action_name) {
    // TODO -- this may possibly be terrible
    self.app.controllers[controller_name][action_name](res);
  };
