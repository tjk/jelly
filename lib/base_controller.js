...
var BaseController = function(app) {
  ...
  this.handle = function(controller_name, action, res, params, ext) {
    /* vars to replace in views are returned by action */
    var vars = this[action](res, params);
    this.render(controller_name, action, res, ext, vars);
  };
  // TODO -- move this (partially or fully) to views.js
  this.render = function(controller_name, action, res, ext, vars, layout) {
    // TODO -- for layout: check action (returned in vars?), then check controller (this.layout?)
