...
function load_layouts(__app_root, __dir) {
  var content = {}, layouts, layout_name;
  layouts = fs.readdirSync(__app_root + __dir);
  if (layouts && layouts.length) {
    for (i in layouts) {
      /* get all layout files that start with a letter */
      if (/^[a-z]/i.test(layouts[i])) {
        /* this seems like a decent way */
        if (layouts[i].indexOf('.') > 0) {
          layout_name = layouts[i].split('.')[0];
          /* could too big of layouts mess this up? */
          content[layout_name] = fs.readFileSync(__app_root + __dir + '/' + layouts[i]);
        }
      }
    }
  }
  console.log(__dir + " -> " + require("util").inspect(content));
  /* maintain reference to layouts */
  return content;
}

/* Store reference to view files to serve -- not modules to require */
function load_views(__app_root, __dir) {
  var content = {}, dir_path, controller_name, view_name;
  ...
  if (dirs && dirs.length) {
    for (i in dirs) {
      ...
      views = fs.readdirSync(dir_path);
      for (j in views) {
        /* get all view files that start with a letter */
        if (/^[a-z]/i.test(views[j])) {
          /* this seems like a decent way */
          if (views[j].indexOf('.') > 0) {
            view_name = views[j].split('.')[0];
            /* could too big of templates mess this up? */
            content[controller_name][view_name] = fs.readFileSync(dir_path + '/' + views[j]);
          }
        }
      }
    }
