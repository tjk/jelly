...
function load_modules(__app_root, __dir) {
  var key, filenames, modules = {};
  filename = fs.readdirSync(__app_root + __dir);
  if (!filenames) return modules;
  for (i in filenames) {
    /* get all files that start with a letter */
    if (/^[a-z]/i.test(filenames[i]) && filenames[i].indexOf('.') > 0) {
...
function load_layouts(__app_root, __dir) {
  var layouts;
  layouts = load_templates_from_dir(__app_root + __dir);
...
function load_views(__app_root, __dir) {
  var dir_path, controller_name, view_name, dirs, views = {};
...
function load_templates_from_dir(path) {
  var files, template, loaded_templates = {};
