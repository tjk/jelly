...
function load_templates(__app_root, __dir) {
  ...
  if (dirs && dirs.length) {
    for (i in dirs) {
      var dirpath = __app_root + __dir + '/' + dirs[i];
      views = fs.readdirSync(dirpath);
      for (j in views) {
        /* get all view files that start with a letter */
        if (/^[a-z]/i.test(views[i])) {
          // TODO -- return what? templates[c][a][f] ? controller, action, fmt
          // TODO -- finish loading the views -- I'M RIGHT HERE ...
          key = views[i].split('.')[0];
