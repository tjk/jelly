  ...
  this.setup = function() {
    return function route(req, res, next) {
      ...
      if (self.routes[pathname] != undefined) {
        ...
        var ca = self.routes[pathname].split('#'), c, a = "index"; // default to index action
        if (ca[0] !== '') c = ca[0];
        if (ca[1] != undefined && ca[1] !== '') a = ca[1];
        try {
          self.app.controllers[c].handle(c, a, res, params); // TODO -- can params be undefined?
        } catch (err) {
        ...
        // TODO -- if /\// or /index.html?/ --> alias for /index.html (to map to index)
