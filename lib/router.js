  ...
  this.setup = function() {
    return function route(req, res, next) {
      var parsed_url = url.parse(req.url, true)
        ...
        , ext = (i < 0) ? "" : pathname.substr(i+1)
        , params   = parsed_url.query
      if (self.routes[path] != undefined) {
        /* split to determine controller#action */
        var ca = self.routes[path].split('#'), c, a = "index"; // default to index action
        if (ca[0] !== '') c = ca[0];
        if (ca[1] != undefined && ca[1] !== '') a = ca[1];
        try {
          self.app.controllers[c].handle(c, a, res, params, ext);
        } catch (err) {
        ...
        c = path.substring(1);
        try {
          self.app.controllers[c].handle(c, "index", res, params, ext);
        } catch (err) {
          self.app.crash("Could not route -- tried { controller => \"" + c +
                     "\", action => \"index\" }.", res, err);
        }
      } else {
        /* check if we can serve it from the public dir (including public/stylesheets/, ...) */
        if (pathname === '/' || pathname === "/index.htm")
          pathname = "/index.html";
        fs.readFile(self.app.app_root + "/public" + path, function(err, data) {
          if (err) app.errors._404(res, err); // throw 404
          else {
            res.writeHead(200, { "Content-Type": self.content_type(pathname) }); // get content-type
            res.write(data);
            res.end();
          }
        });
