...
var Router = function(app) {
  ...
  this.setup = function() {
    return function route(req, res, next) {
      if (this.app.log) log_access(req);
      ...
      if (self.routes[pathname] != undefined) {
        ...
        try {
          this.app.controllers[c].handle(c, a, res);
        } catch (err) {
          this.app.crash("Could not route -- tried { controller => " + c +
                     ", action => " + a + " }.", res, err);
        }
      } else if (["/", "/index.htm", "/index.html"].indexOf(pathname) >= 0) {
        // not path to index -- server public/index.html if available
        fs.readFile(this.app.app_root + "/public/index.html", function(err, data) {
